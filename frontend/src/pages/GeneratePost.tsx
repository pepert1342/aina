...
function GeneratePost() {
  ...
  const handleGenerateAll = async () => {
    await Promise.all([handleGenerate(), handleGenerateImage()]);
  };
  ...
  const handleDownloadImage = () => {
    ...
    link.download = `post-${event?.title || 'image'}.png`;
    link.click();
  };
  ...
  return (
    <div style={{
      ...
      <main style={{
        ...
        {/* Event Info Banner - Dégradé comme le logo */}
        {event && (
          <div style={{
            background: 'linear-gradient(135deg, #FF6B35, #004E89)',
            borderRadius: '20px',
        ...
        <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <button
                  ...
                  style={{
                    padding: '20px',
                    borderRadius: '14px',
                    border: `3px solid ${selectedFormat === 'carre' ? '#FF6B35' : '#E5E7EB'}`,
                    backgroundColor: selectedFormat === 'carre' ? '#FF6B3515' : 'white',
                    cursor: 'pointer',
                    display: 'flex',
                ...
                <button
                  ...
                  style={{
                    padding: '20px',
                    borderRadius: '14px',
                    border: `3px solid ${selectedFormat === 'story' ? '#FF6B35' : '#E5E7EB'}`,
                    backgroundColor: selectedFormat === 'story' ? '#FF6B3515' : 'white',
                    cursor: 'pointer',
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: 'center',
          ...
          {/* Deuxième ligne : Boutons d'action GROS et alignés */}
          <div style={{ 
            display: 'flex', 
            gap: '16px',
            paddingTop: '24px',
            ...
            <button
              ...
              style={{
                flex: 1,
                padding: '20px 32px',
                background: event && !generating ? 'linear-gradient(135deg, #FF6B35, #FF8F5E)' : '#E5E7EB',
                border: 'none',
                borderRadius: '16px',
                color: event && !generating ? 'white' : '#999',
                fontWeight: '700',
                cursor: event && !generating ? 'pointer' : 'not-allowed',
                fontSize: '18px',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                gap: '10px',
                boxShadow: event && !generating ? '0 6px 20px rgba(255, 107, 53, 0.3)' : 'none',
            ...
            <button
              ...
              disabled={!event || generatingImage}
              style={{
                flex: 1,
                padding: '20px 32px',
                background: event && !generatingImage ? 'linear-gradient(135deg, #FF6B35, #FF8F5E)' : '#E5E7EB',
                border: 'none',
                borderRadius: '16px',
                color: event && !generatingImage ? 'white' : '#999',
                fontWeight: '700',
                cursor: event && !generatingImage ? 'pointer' : 'not-allowed',
                fontSize: '18px',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                gap: '10px',
                boxShadow: event && !generatingImage ? '0 6px 20px rgba(255, 107, 53, 0.3)' : 'none',
                transition: 'all 0.3s ease'
              }}
            >
            ...
            <button
              ...
              disabled={!event || generating || generatingImage}
              style={{
                flex: 1,
                padding: '20px 32px',
                background: event && !generating && !generatingImage
                  ? 'linear-gradient(135deg, #FF6B35, #004E89)'
                  : '#E5E7EB',
                border: 'none',
                borderRadius: '16px',
                color: event && !generating && !generatingImage ? 'white' : '#999',
                fontWeight: '700',
                cursor: event && !generating && !generatingImage ? 'pointer' : 'not-allowed',
                fontSize: '18px',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                gap: '10px',
                boxShadow: event && !generating && !generatingImage 
          ...
          <div style={{
            ...
            {generatedTexts.length === 0 ? (
              <div style={{ 
                ...
                background: 'linear-gradient(135deg, #FFF5F2, #FFFFFF)',
                borderRadius: '16px',
                border: '2px dashed #FFE5DC'
              }}>
                <div style={{ fontSize: '64px', marginBottom: '16px' }}>📝</div>
                <p style={{ fontSize: '16px', fontWeight: '500' }}>Les textes apparaîtront ici</p>
                <p style={{ fontSize: '14px', marginTop: '8px' }}>Cliquez sur "Texte" ou "Tout Générer"</p>
              </div>
            ) : (
                ...
                {generatedTexts.map((text, index) => (
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: selectedVersion === index ? '#FF6B3508' : 'white',
                      cursor: 'pointer',
                      transition: 'all 0.3s ease'
                    }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                      ...
                      {selectedVersion === index && (
                        <span style={{ backgroundColor: '#FF6B35', color: 'white', padding: '4px 12px', borderRadius: '50px', fontSize: '12px', fontWeight: '600' }}>
                          ✓ Sélectionnée
                        </span>
                      )}
                    </div>
